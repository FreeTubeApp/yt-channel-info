...
class YoutubeGrabberValidation {
     ...
     * */
    static isResourceIsChannel = (url) => url.includes('/user/') || url.includes('/channel/');
     ...
     * */
    static isResourceIsPlaylist = (url) => url.includes('playlist?list=');
     ...
     * */
    static isResourceIsVideo = (url) => url.includes('watch?v=') || url.includes('/v/');
    ...
    static getChannelIdOrUser(url) {
      ...
      if (regUser)
        return regUser[2].endsWith('/') ? regUser[2].substring(0, regUser[2].length - 1) : regUser[2];
